...
const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    // Hidden trigger for admin panel: Ctrl + Shift + A
  ...
  const renderInterestIcon = (id: string) => {
    ...
    const size = 24;
    switch (id) {
      case 'i1': return <Cpu size={size} className={className} />;
      case 'i2': return <PieChart size={size} className={className} />;
      case 'i3': return <Users size={size} className={className} />;
      case 'i4': return <Lightbulb size={size} className={className} />;
      case 'i5': return <TrendingUp size={size} className={className} />;
      case 'i6': return <Anchor size={size} className={className} />;
          ...
          <div className="flex flex-col-reverse md:flex-row items-center gap-12 md:gap-24 justify-between">
            ...
            <div className="relative shrink-0">
               <div className="w-64 h-64 md:w-96 md:h-96 rounded-full border-4 border-slate-800 shadow-2xl overflow-hidden relative z-10 group bg-slate-800">
                  {data.profileImage ? (
                    <img src={data.profileImage} alt={data.name} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-700" />
                  ) : (
           ...
           <div className="max-w-4xl mx-auto relative z-20">
              ...
              <div className="bg-slate-50/50 p-8 md:p-12 rounded-3xl border border-slate-100 shadow-sm relative group">
                ...
                <div className="prose prose-lg max-w-none text-slate-700 leading-relaxed font-light">
                  {data.summary.split('\n').map((paragraph, i) => <p key={i} className="mb-8 last:mb-0">{paragraph}</p>)}
                </div>
                <div className="mt-12 flex flex-col items-end">
                   <div className="text-xl md:text-2xl font-hand text-slate-900 transform -rotate-3 hover:rotate-0 transition-transform duration-500 cursor-default">Rob</div>
                </div>
              </div>
           </div>
         </div>
           ...
           <div className="max-w-6xl mx-auto relative z-20">
             <div className="text-center mb-16">
               ...
               <div className="relative z-20">
                 <h2 className="text-4xl font-bold text-slate-900 mb-4 inline-block"><span className="bg-slate-50/95 px-4 py-1 rounded-lg">Professional Journey</span></h2>
                 <br />
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-16 md:gap-24">
              <div>
                 <div className="flex items-center gap-4 mb-10 border-b pb-6">
                   <div className="w-10 h-10 bg-slate-100 rounded-lg flex items-center justify-center text-slate-500"><Quote size={20} /></div>
                ...
                <div className="space-y-12">
                  {data.education.map(edu => (
                    <div key={edu.id}>
                      ...
                      <p className="text-slate-400 text-sm">{edu.years}</p>
                    </div>
                  ))}
                </div>
              </div>
              <div>
                <div className="flex items-center gap-4 mb-10 border-b pb-6">
                   <div className="w-10 h-10 bg-slate-100 rounded-lg flex items-center justify-center text-slate-500"><Award size={20} /></div>
                  ...
                  {data.awards.map(award => (
                    <div key={award.id} className="flex gap-4 items-start">
                      ...
                      <div>
                        ...
                        {award.description && <p className="text-slate-500">{award.description}</p>}
                      </div>
                    </div>
                  ))}
                </div>
      ...
      <section id="principles" className="h-screen w-full snap-start flex flex-col bg-slate-900 text-white relative overflow-hidden">
         ...
         <div className="flex-1 w-full overflow-y-auto px-6 py-24 flex items-center justify-center relative z-20 no-scrollbar">
           <div className="max-w-6xl mx-auto w-full">
              <h2 className="text-3xl font-bold mb-12 flex items-center gap-4"><span className="w-12 h-1 bg-red-500 rounded-full"></span> My Key Principles</h2>
         ...
         <div className="flex-1 w-full overflow-y-auto px-6 py-24 flex items-center justify-center no-scrollbar">
           <div className="max-w-6xl mx-auto w-full relative z-20">
              <div className="flex items-center gap-4 mb-12">
                 <div className="p-3 bg-white rounded-lg text-red-600 shadow-sm border border-slate-100"><Heart size={24} /></div>
                 <h2 className="text-3xl font-bold text-slate-900">Personal & Professional Interests</h2>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {data.interests.map(i => (
                  <div key={i.id} className="bg-white p-8 rounded-xl border border-slate-100 shadow-sm hover:shadow-md transition-shadow group">
         ...
         <div className="flex-1 w-full overflow-y-auto px-6 py-24 flex items-center justify-center no-scrollbar">
           <div className="max-w-4xl mx-auto w-full relative z-20 text-center">
              <div className="mb-16">
                ...
                <p className="text-slate-400 text-lg">Interested in working together? Drop me a message.</p>
              </div>
              <div className="flex flex-col items-center gap-8 relative z-20">
                 ...
                 >
                   <Send size={24} className="group-hover:translate-x-1 group-hover:-translate-y-1 transition-transform" /> Send Message
                 </button>
                 <div className="flex gap-4 mt-8">
                    <button onClick={() => initiateAction('download')} className="flex items-center gap-2 px-6 py-3 bg-red-600 hover:bg-red-700 text-white font-medium rounded-full transition-all shadow-lg shadow-red-900/20"><Download size={18} /> Download CV</button>
                    <a href={`https://${data.contact.linkedin}`} target="_blank" rel="noreferrer" className="flex items-center gap-2 px-6 py-3 bg-slate-800 hover:bg-[#0077b5] text-white font-medium rounded-full transition-all border border-slate-700 hover:border-[#0077b5]"><Linkedin size={18} /> Connect on LinkedIn</a>
                 </div>
                 <p className="text-slate-600 text-sm mt-12">© {new Date().getFullYear()} {data.name}. All rights reserved.</p>
              </div>
           </div>
         </div>
      </section>
