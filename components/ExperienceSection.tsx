      ...
      <div className="relative">
        ...
        {filteredExperience.length === 0 ? (
          <div className="text-center py-12 text-slate-500 bg-white rounded-xl border border-dashed border-slate-300 mx-4">
            <p>No roles found matching these filters.</p>
            <button 
              onClick={() => setActiveTags([])}
              className="text-red-500 font-bold mt-2 hover:underline"
            >
              Reset Filters
            </button>
          </div>
                ...
                <div className="bg-white rounded-xl shadow-lg shadow-slate-200/50 border border-slate-100 overflow-hidden hover:shadow-xl transition-all duration-300 hover:-translate-y-1 group relative z-20">
                  ...
                  <div className="p-6 border-b border-slate-50 bg-slate-50/50">
                    <div className="flex flex-wrap gap-2 mb-3">
                       {exp.tags?.map((tag, i) => (
                         <span key={i} className="px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider bg-slate-200 text-slate-600">
                           {tag}
                         </span>
                       ))}
                    </div>
                      ...
                      <div className="pt-4 border-t border-slate-50">
                        ...
                        <div className="flex flex-wrap gap-2">
                          {exp.skills.map((skill, i) => (
                            <span key={i} className="px-2.5 py-1 rounded-md bg-white border border-slate-200 text-slate-600 text-[10px] font-bold hover:border-red-200 hover:bg-red-50 hover:text-red-600 transition-colors cursor-default">
                              {skill}
                            </span>
                          ))}
                        </div>
